...
## Context
A bug report: creating a **new Run** shows the **same stressors and tasks** as the previous Run. The user expects each Run to be a separate set of stressors/tasks (*"each run should be a separate set of tasks and stressors"*), but every Run edits and displays one shared funnel.
