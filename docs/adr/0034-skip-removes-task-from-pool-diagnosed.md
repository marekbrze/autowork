...
## Context
A bug report: skipping a focus task removes it from the available pool — behaving like Dismiss ("Not relevant") instead of the intended temporary deferral (*"not now, I'll come back at the next session"*). Needed a root-cause diagnosis before fixing.
