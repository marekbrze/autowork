...
| Navigate to funnel step | Skok do dowolnego kroku aktywnego Runa przez klikalny stepper na ekranach lejka (Stresory/Ranking/Akcje/Procesowanie/Focus). | User | Bieżący krok = no-op. W aktywnej sesji focus → ConfirmDialog (pauza, snapshot per-Run, wznawialny przez `SessionResumeBanner`). Nie zmienia `lastReachedStep` (Continue nadal wg danych). Supersede ADR 0001; ADR 0048. |
